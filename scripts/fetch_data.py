...
def load_pe_history() -> List[dict]:
    ...
    clean = []
    for item in items:
        if "date" in item and "pe" in item:
            clean.append({"date": item["date"], "pe": float(item["pe"])})
...
def pe_percentile(day: str, pe_value: float, pe_items: List[dict], config: dict) -> float:
    ...
    start_day = current - timedelta(days=window_days * 2 // 1)  # 日历天近似，足够宽松
...
def score_record(raw: dict, config: dict) -> dict:
    ...
    vix_score = clamp(
        ((float(raw["vix"]) - float(config["vol"]["floor"])) / (float(config["vol"]["cap"]) - float(config["vol"]["floor"]))) * float(weights["vol"]),
        0.0,
        float(weights["vol"]),
    )
